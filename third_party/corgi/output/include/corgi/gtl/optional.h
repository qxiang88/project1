...
#ifndef CORGI_GTL_OPTIONAL_H_
#define CORGI_GTL_OPTIONAL_H_
...
template <class T>
struct hash<::corgi::gtl::optional<T>> {
  size_t operator()(const ::corgi::gtl::optional<T>& opt) const {
    if (opt) {
      return hash<T>()(*opt);
    } else {
